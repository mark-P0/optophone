...
def reorder_points(
    ...
    top_left, *_ = [
        (x, y)
        for x, y in points
        if x in x_left and y in y_top]
    bot_left, *_ = [
        (x, y)
        for x, y in points
        if x in x_left and y in y_bot]
    bot_right, *_ = [
        (x, y)
        for x, y in points
        if x in x_right and y in y_bot]
    top_right, *_ = [
        (x, y)
        for x, y in points
        if x in x_right and y in y_top]
...
def dewarp(img_mat: cv.Mat) -> cv.Mat:
    ...
    shape_corners = cv.approxPolyDP(
        largest_contour, epsilon, True)

    ## Normalize shape corners and set as base shape
    ## Currently: [[x, y], ...]`
    base_shape = reorder_points(
        [corner[0] for corner in shape_corners])
    ...
    ### Width, height naively computed
    ### as average of corresponding sides...
    ...
    M = cv.getPerspectiveTransform(
        base_shape, target_shape)
    warped_mat = cv.warpPerspective(
        img_mat,
        M,
